...
class MyKeyInfo {
    ...
    getKey(): Buffer {
        throw new Error("Key retrieval is not supported.");
    }
}
...
export const signInvoice = async (xml: string) => {
    try {
        // console.log('[INFO] Reading private key file...');
        let privateKey = fs.readFileSync('./src/services/SDI-PKCS8-04126420043.pem', 'utf8');
        // console.log('[DEBUG] Private Key Content:', privateKey);
        if (!privateKey.includes('BEGIN PRIVATE KEY')) {
            throw new Error('Invalid Private Key format - Ensure it is in PKCS8 format');
        }

        // console.log('[INFO] RsigningKeyeading certificate file...');
        const certificate = fs.readFileSync('./src/services/SDI-04126420043.pem', 'utf8');
...
// console.log("Signing done", signature);
        ...
        // (sig as any).keyInfoProvider = new MyKeyInfo(certificate);
        // (sig as any). = Buffer.from(privateKey.trim(), 'utf8');

        // console.log('[DEBUG] signingKey:', (sig as any).signingKey);
